...
class GithubClient:
    ...
    def get_changes(self, start: datetime, end: datetime) -> list[Change]:
        ...
        if ChangeType.COMMIT in set(self.config.include_types):
            ...
            # Use proper date filtering for commits now that date boundaries are correct
            changes.extend(
    ...
    def _fetch_contributions(self, start: datetime, end: datetime) -> dict[str, Any]:
        ...
        if "data" in data and "user" in data["data"]:
            ...
            if user_data and "contributionsCollection" in user_data:
                ...
                logger.debug(
                    "  Contribution counts: issues=%d, prs=%d, reviews=%d, "
                    "commit_repos=%d, restricted=%d",
                    issues_count,
                    prs_count,
                    reviews_count,
                    len(commit_repos),
                ...
                for i, repo_data in enumerate(commit_repos):
                    ...
                    # Log first few contribution timestamps to understand
                    # GitHub's calendar logic
