...
class GraphQLClient:
    ...
    def collect_reviews(self, coll: dict[str, Any]) -> list[Change]:
        ...
        results: list[Change] = []
        review_contribs = coll.get("pullRequestReviewContributions", {}).get("nodes", [])
    ...
    def discover_repos_from_contributions(self, coll: dict[str, Any]) -> set[str]:
        ...
        for contrib in coll.get("issueContributions", {}).get("nodes", []):
            repo_name = contrib.get("issue", {}).get("repository", {}).get("nameWithOwner")
            if repo_name and self._repo_allowed(repo_name):
                repos.add(repo_name)
        
        # From pull requests  
        for contrib in coll.get("pullRequestContributions", {}).get("nodes", []):
            repo_name = contrib.get("pullRequest", {}).get("repository", {}).get("nameWithOwner")
            if repo_name and self._repo_allowed(repo_name):
                repos.add(repo_name)
        
        # From reviews
        for contrib in coll.get("pullRequestReviewContributions", {}).get("nodes", []):
            repo_name = contrib.get("pullRequestReview", {}).get("pullRequest", {}).get("repository", {}).get("nameWithOwner")
            if repo_name and self._repo_allowed(repo_name):
                repos.add(repo_name)
