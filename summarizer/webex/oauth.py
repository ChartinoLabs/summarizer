    ...
    def do_GET(self) -> None:
        ...
        if hasattr(self.server, 'oauth_result'):
            if auth_code:
                ...
                self.server.oauth_result = {'error': error, 'description': error_description}
...
class OAuthCallbackServer:
    ...
    def start(self) -> str:
        ...
        if not self.server:
            raise RuntimeError(f"Could not start callback server on ports {self.port}-{self.port + 9}")
...
class WebexOAuthClient:
    ...
    def exchange_code_for_tokens(self, code: str, code_verifier: str) -> WebexOAuthCredentials:
    ...
    def refresh_access_token(self, credentials: WebexOAuthCredentials) -> WebexOAuthCredentials:
    ...
    def get_valid_access_token(self) -> str | None:
        ...
        logger.debug(f"Loaded credentials - expires: {credentials.expires_at}, scopes: {credentials.scope}")
        ...
        token_prefix = credentials.access_token[:10] + "..." if len(credentials.access_token) > 10 else credentials.access_token
    ...
    def start_interactive_auth(self) -> WebexOAuthCredentials:
        ...
        try:
            ...
            try:
                ...
                if 'error' in result:
                    raise RuntimeError(f"Authorization failed: {result['error']} - {result.get('description', '')}")
                
                if 'code' not in result:
                    raise RuntimeError("No authorization code received")
                
                # Exchange code for tokens
                credentials = self.exchange_code_for_tokens(result['code'], code_verifier)
