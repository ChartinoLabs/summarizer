...
class TestWebexConfigOAuth:
    ...
    def test_config_without_credentials(self) -> None:
        ...
        config = WebexConfig(
            user_email="test@example.com", target_date=datetime(2024, 1, 1)
        )
    ...
    def test_get_access_token_oauth_preferred(
        self, mock_oauth_client_class: MagicMock
    ) -> None:
    ...
    @patch("summarizer.webex.config.WebexOAuthClient")
    def test_get_access_token_fallback_to_manual(
        self, mock_oauth_client_class: MagicMock
    ) -> None:
